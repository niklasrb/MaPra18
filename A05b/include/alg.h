...
void Dijkstra(const DistanceGraph& g, GraphVisualizer& v, VertexT start, std::vector<CostT>& kostenZumStart) {
	...
	while(S.size() < g.numVertices()) {
		...
		for(auto it = neighbors.begin(); it != neighbors.end(); ++it) {
			v.markEdge(EdgeT(m, it->first), EdgeStatus::Active);
			auto k = kostenZumStart.at(m) + it->second;
			kostenZumStart.at(it->first) = std::min(k, kostenZumStart.at(it->first));
			v.updateVertex(it->first, kostenZumStart.at(it->first), 0, m, (it->first != start ? (!contains(S, it->first) ? VertexStatus::InQueue : VertexStatus::Done) : VertexStatus::Origin));
			v.markEdge(EdgeT(m, it->first), EdgeStatus::Visited);
		}
		if(m != start) v.markVertex( m, VertexStatus::Done);
	}
}
...
bool A_star(const DistanceGraph& g, GraphVisualizer& v, VertexT start, VertexT ziel, PathT& weg) {
    ...
    while(open.size() > 0) {
		...
		for(auto it = g.getNeighbors(cur).begin(); it != g.getNeighbors(cur).end(); ++it) {
			...
			if(moegliche_gKosten >= gKosten.at(it->first))  {
				v.markEdge(e, EdgeStatus::Visited);
			...
			v.updateVertex(it->first, moegliche_gKosten, g.estimatedCost(it->first, ziel), cur, 
											(it->first == start ? VertexStatus::Origin : (it->first == ziel ? VertexStatus::Destination : VertexStatus::InQueue)));
			v.markEdge(e, EdgeStatus::Visited);

		}
		closed.push_back(cur);
		if(cur != start && cur != ziel) v.markVertex(cur, VertexStatus::Done);
	}
